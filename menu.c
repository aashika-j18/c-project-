...
int main()
{
    ...
    //print_adv_cleared(adv_cleared_len,adv_cleared);
    ...
    while (admin_or_student < 3)
    {
                ...
                if (allocation_started == 0)
                {
                    ...
                    while ((choice > 0) && (choice < 3))
                    {
                        ...
                        if (choice == 1)
                        {
                            ...
                            // storing inst details(over here because at the ned seats will become 0)
                            // print_inst_details(iit_details_len,iit_details);
                            //store_inst_details("IIT_Details_temp.csv", iit_details_len, iit_details);
                            //store_inst_details("NIT_Details_temp.csv", nit_details_len, nit_details); 
                        }
                        else if (choice == 2)
                        {
                            int allocate = 0;
        ...
        {
            int reqd_roll_no = 6;
            if (student_login(&reqd_roll_no, students_only_len, students_only))
            {
