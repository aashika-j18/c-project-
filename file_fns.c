    ...
    if (token != NULL)
    {
      ...
      if (token != NULL)
      {
        ...
        if (token != NULL)
        {
          ...
          if (token != NULL)
          {
            // strlen-1 because \n char is getting copied
            strncpy(mains_cleared[*mains_cleared_len].allocated_branch, token, strlen(token) - 1);
            (mains_cleared[*mains_cleared_len].allocated_branch)[strlen(token) - 1] = '\0';
          }
          else
          {
...
int store_adv_cleared(int adv_cleared_len, struct adv_cleared_struct adv_cleared[])
{
  ...
  // Open the CSV file for writing
  file = fopen("adv_cleared_temp.csv", "w");
  if (file == NULL)
  {
    printf("Unable to open file %s\n", "adv_cleared.csv");
  }
...
int store_mains_cleared(int mains_cleared_len, struct mains_cleared_struct mains_cleared[])
{
  ...
  // Open the CSV file for writing
  file = fopen("mains_cleared_temp.csv", "w");
  if (file == NULL)
  {
    printf("Unable to open file %s\n", "mains_cleared.csv");
  }
  ...
  for (int i = 0; i < mains_cleared_len; i++)
  {
    ...
    if (mains_cleared[i].allocated_inst != "")
    {
